...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // 4. Low Stock warnings (stock <= 5 in active warehouse or globally)
        if ($activeGudang === 'all') {
            ...
            $lowStock = StokGudang::where('kode_gudang', $activeGudang)
                ->where('stok_sekarang', '<=', 5)
                ->count();
        }

        return view('dashboard', compact('totalBarang', 'incomingToday', 'outgoingToday', 'lowStock'));
    }
}
