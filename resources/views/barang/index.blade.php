...
@section('title', 'Master Barang - Inventory Gudang')
...
<div class="flex flex-wrap -mx-3">
    <div class="w-full max-w-full px-3">
        ...
        <div class="relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border mb-6">
            <div class="p-6">
                <h6 class="font-bold text-slate-800 text-sm mb-3">Import Data Barang & Stok Awal dari Excel</h6>
                <form action="{{ route('barang.import') }}" method="POST" enctype="multipart/form-data" class="flex flex-wrap items-end gap-4">
                    ...
                    <div class="flex flex-col gap-1 w-full max-w-sm">
                        <label class="text-xxs font-bold text-slate-700 uppercase pl-1">Pilih Berkas Excel (.xlsx) <span class="text-red-500">*</span></label>
                        <div class="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-xl p-1.5 w-full h-[42px]">
                            <i class="fa fa-file-excel text-emerald-500 text-sm ml-1.5 mr-1"></i>
                            <input type="file" name="file" accept=".xlsx" required class="text-xs text-slate-500 file:mr-2 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:text-xs file:font-bold file:bg-white file:text-slate-600 hover:file:bg-gray-100 cursor-pointer w-full">
                        </div>
                    </div>

                    <!-- Submit Button -->
                    <button type="submit" class="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-colors rounded-lg cursor-pointer bg-emerald-600 hover:bg-emerald-700 leading-pro text-xs tracking-tight h-[42px]">
                        <i class="fa fa-upload mr-1"></i> Mulai Import
                    </button>
                </form>
            </div>
        </div>
        ...
        <div class="relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
            ...
            <div class="p-6 pb-0 mb-0 border-b-0 border-solid border-black-125 rounded-t-2xl flex flex-wrap justify-between items-start gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3 ml-auto">
                    ...
                    <!-- Add Barang Button -->
                    @if(Auth::user() && Auth::user()->isSuperAdmin())
                    <a href="{{ route('barang.create') }}" class="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-colors rounded-lg cursor-pointer bg-blue-600 hover:bg-blue-700 leading-pro text-xs tracking-tight">
