        ...
        <div class="relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
            ...
            <div class="p-6 pb-0 mb-0 border-b-0 border-solid border-black-125 rounded-t-2xl flex flex-wrap justify-between items-start gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3 ml-auto">
                    ...
                    <!-- Add Button -->
                    <a href="{{ route('barang-masuk.create') }}" class="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-blue-600 to-sky-400 leading-pro text-xs ease-soft-in shadow-soft-md hover:shadow-soft-2xl hover:scale-102 active:opacity-85 tracking-tight">
                        <i class="fa fa-plus mr-1"></i> Catat Barang Masuk
                    </a>
                </div>
            </div>
                ...
                <div class="p-0 overflow-x-auto">
                    <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                        ...
                        <tbody>
                            ...
                            <tr class="tx-row" data-jenis="{{ $tx->jenis_transaksi }}" data-search="{{ strtolower($tx->no_surat_jalan) }} {{ strtolower($tx->pengirim) }} {{ strtolower($tx->gudang ? $tx->gudang->nama_gudang : '') }} {{ strtolower($tx->user ? $tx->user->name : 'system') }}">
                                <td class="px-6 py-4 align-middle bg-transparent border-b whitespace-nowrap shadow-none">
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        $(document).on("click", "#nextPageBtn", function() {
            let totalMatching = $(".tx-row").filter(function() {
                ...
                let rowSearchText = $row.data("search") || '';
                let rowJenis = $row.data("jenis") || '';
                return (rowSearchText.indexOf($("#searchInput").val().toLowerCase().trim()) > -1) && ($("#filterJenis").val() === "" || rowJenis === $("#filterJenis").val());
            }).length;
