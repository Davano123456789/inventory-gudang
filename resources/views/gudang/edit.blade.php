...
@section('title', 'Edit Gudang - Inventory Gudang')
    ...
    <div class="w-full max-w-full px-3">
        ...
        <div class="relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
            ...
            <div class="p-6 pb-0 mb-0 border-b-0 border-solid border-black-125 rounded-t-2xl flex justify-between items-center">
                <h6 class="font-bold text-slate-800 text-lg">Form Edit Gudang</h6>
            ...
            <div class="flex-auto p-6">
                <form action="{{ route('gudang.update', $gudang->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="kode_gudang" id="kode_gudang" value="{{ old('kode_gudang', $gudang->kode_gudang) }}" required class="w-full px-3 py-2 text-sm text-slate-700 bg-white border border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none transition-colors">
                        <p class="text-xs text-slate-400 mt-1">Harap berhati-hati saat mengubah kode gudang karena terhubung dengan transaksi.</p>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_gudang" id="nama_gudang" value="{{ old('nama_gudang', $gudang->nama_gudang) }}" required class="w-full px-3 py-2 text-sm text-slate-700 bg-white border border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none transition-colors">
                    </div>

                    <div class="mb-6">
                        <label for="alamat" class="block mb-2 text-xs font-bold text-slate-700 uppercase">Alamat Gudang</label>
                        <textarea name="alamat" id="alamat" rows="4" class="w-full px-3 py-2 text-sm text-slate-700 bg-white border border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none transition-colors">{{ old('alamat', $gudang->alamat) }}</textarea>
                    </div>
                    ...
                    <div class="flex justify-start gap-3 border-t pt-4">
                        <button type="submit" class="px-6 py-3 font-bold text-white uppercase bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors text-xs tracking-wider">
                            <i class="fa fa-save mr-1"></i> Perbarui Gudang
                        </button>
